...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof isPersymmetricMatrix, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns `true` if provided a persymmetric matrix', function test( t ) {
	...
	t.strictEqual( isPersymmetricMatrix( arr ), true, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns `false` if not provided a square matrix', function test( t ) {
	...
	t.strictEqual( isPersymmetricMatrix( arr ), false, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns `false` if not provided a 2-dimensional ndarray-like object', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.strictEqual( isPersymmetricMatrix( values[i] ), false, 'returns false when provided '+values[i] );
	}
	t.end();
});
